...
class UnQuotedSyntaxToken(SyntaxToken):
    def format_value(self) -> str:
        # Leave unquote
        return self.value + ','
...
class SingleNode(SyntaxNode):
    ...
    def __str__(self) -> str:
        return cube_to_str(self.type, " ", self.value)
...
class ListNode(SyntaxNode):
    ...
    type: SyntaxToken
    items: Union[Tuple[PairNode, ...], Tuple[SingleNode, ...], Tuple[SyntaxToken, ...]]
    ...
    @property
    def children(self,) -> Union[Tuple[PairNode, ...], Tuple[SingleNode, ...]] :
        ...
        if self.items and isinstance(self.items[0], SingleNode):
            # Assume that all elements are pairs
            return cast(Tuple[SingleNode, ...], self.items)
        else:
            return tuple()
...
class ContainerNode(SyntaxNode):
    ...
    items: Tuple[Union[BlockNode, PairNode, SingleNode, ListNode], ...]
    ...
    @property
    def children(self) -> Tuple[Union[BlockNode, PairNode, SingleNode, ListNode], ...]:
        return self.items
...
class BlockNode(SyntaxNode):
    ...
    def __str__(self) -> str:
        ...
        return items_to_str(
            self.type, self.colon, name, self.left_brace, container, self.right_brace, self.trailing_comma
        )
...
class Visitor(ABC):
    ...
    @abstractmethod
    def visit_pair(self, node: SingleNode) -> Any:
        ...
