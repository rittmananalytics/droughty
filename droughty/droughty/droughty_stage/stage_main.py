...
# TODO improve to use droughty_project.yaml to specify output path

def droughty_stage_main(gcp_project_id=Common.project_id, target_datasets=Common.datasets, target_table=Common.table):
    """This script will generate compatiable sources.yml files + .sql files (raw + clean + dimensions)
    for each table within the datasets of the GCP project specified. Note this should only be
    used against datasets not containing objects generated by dbt; e.g datasets generated by fivetran.
    These yml files will be stored in analysis/utilties/schema_files"""
    ...
    output_dir = os.path.join(scriptFolder, "schema_files/")
    ...
    if not os.path.exists(os.path.dirname(scriptFolder+"dump/shDump/")):
        os.makedirs(os.path.dirname(scriptFolder+"dump/shDump/"), exist_ok=True)

    if not os.path.exists(os.path.dirname(scriptFolder+"dump/jsonDump/")):
        os.makedirs(os.path.dirname(scriptFolder+"dump/jsonDump/"), exist_ok=True)

    if not os.path.exists(os.path.dirname(scriptFolder+"dump/tableDump/")):
        os.makedirs(os.path.dirname(scriptFolder+"dump/tableDump/"), exist_ok=True)
    ...
    for targetSource in target_datasets:
        ...
        if system in ['darwin', 'linux']:
            #####
            with open("{}{}{}_{}.sh".format(scriptFolder, "dump/shDump/", "tables", targetSource), "w") as file:
                file.write(f"{ProjectVariables.bq_sdk_path} ls --max_results=10000 {gcp_project_id}:{targetSource} | awk '{{print $1}}' | tail +3  > {scriptFolder}dump/tableDump/tableList_{targetSource}.txt")

                

            os.system("sh {}{}{}_{}.sh".format(
                scriptFolder, "dump/shDump/", "tables", targetSource))

            with open("{}{}tableList_{}.txt".format(scriptFolder, "dump/tableDump/", targetSource), "r") as tables:
                if output:
            ...
            with open("{}{}{}_{}.sh".format(scriptFolder, "dump/shDump/", "tableCols", targetSource), "w") as f:
                for dataset in tableList.keys():
                    for table in tableList[dataset]:
                        if output:
                            print("Extracting columns from: {}.{}".format(
                                targetSource, table))
                        f.write(f"{ProjectVariables.bq_sdk_path} show --schema {gcp_project_id}:{dataset}.{table} > {scriptFolder}dump/jsonDump/{dataset}.{table}.json\n")
    ...
    for enum, file in enumerate(os.listdir(scriptFolder+"dump/jsonDump/")):
        ...
        with open(scriptFolder+"dump/jsonDump/"+"/"+file, "r") as f:
            data = json.load(f)
            with open("{}{}{}{}".format(scriptFolder, "schema_files/", dsColFolder, "sources.txt"), 'a+') as columnTxt:
                if output:
            ...
            # stage #
            if not os.path.exists(os.path.dirname("{}{}{}stage/".format(scriptFolder, "schema_files/", dsColFolder))):
                os.makedirs(os.path.dirname(
                    "{}{}{}stage/".format(scriptFolder, "schema_files/", dsColFolder)), exist_ok=True)

            with open("{}{}{}stage/{}".format(scriptFolder, "schema_files/", dsColFolder, datasetName+"__"+tableNameOrig+".sql"), 'w') as columnTxt:
                if output:
